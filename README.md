...
6.  Мигрируйте дб. Phpmyadmin доступна по адресу `http://localhost:8000`. Для миграции подойдет дб с дев мультисайта. Используя wp migrate db поставьте `//localhost` для реплейса домена. Если не используете этот плагин, то в дб перезапишите старый домен `//olddomain.com` на локалхост `//localhost`.
...
9.  Перенесите плагины в папку `wp/wp-content/plugins`. Копируйте их с дев сайта с которого мигрировали дб. **Не переноcите** плагины что подключены как git submodule.
10. Сайт будет доступен по адресу `https://localhost`. Вы можете использовать другие доменные имена дополнив ваш hosts файл на windows и перезаписав домен в дб.
11. Для установки зависимостей фронтенда используйте команду:
    ```
    yarn install
    ```
