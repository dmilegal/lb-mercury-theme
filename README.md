...
5.  Мигрируйте дб. Phpmyadmin доступна по адресу `http://localhost:8000`. Для миграции подойдет дб с дев мультисайта. Используя wp migrate db поставьте `//localhost` для реплейса домена. Если не используете этот плагин, то в дб перезапишите старый домен `//olddomain.com` на локалхост `//localhost`.
6.  Создайте `wp/.htaccess` если он еще не создан. Это файл должен содержать следующий код:
...
8.  Перенесите плагины в папку `wp/wp-content/plugins`. Копируйте их с дев сайта с которого мигрировали дб. **Не переноcите** плагины что подключены как git submodule.
